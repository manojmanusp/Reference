$(document).ready(function () {
    if ($("input#addListItemJSOM").length == 0) {
        ...
        var UpdateControlJSOM = '<input type="button" id="updateListItemJSOM" name="updateListItemJSOM" value="Update Item JSOM" class="ms-ButtonHeightWidth">';
        JSRequest.EnsureSetup();
        var idValue = JSRequest.QueryString["ID"];    
        $("input[id$='diidIOSaveItem']").parent().append(UpdateControlJSOM);
        ...
        $("input[name='updateListItemJSOM']").click(function () {
            ...
            function UpdateNewItemJSOM() {
                ...
                GetUserIDByNameJSOM(managersValue).then(function (userInfo1) {
                    UpdateItemMessageListJSOM(title, contactPerson, messageDetails, dateMessage, emailValue, statusValue, userInfo1, idValue);
                });
        ...
        var cancelEditControlJSOM = '<input type="button" id="cancelEditListItemJSOM" name="cancelEditListItemJSOM" value="Cancel Item JSOM" class="ms-ButtonHeightWidth">';

        $("input[name='updateListItemJSOM']").parent().append(cancelEditControlJSOM);

        //Click event for custom Cancel button
        $("input[name='cancelEditListItemJSOM']").click(function () {
...
function UpdateItemMessageListJSOM(title, contactPerson, messageDetails, dateMessage, emailValue, statusValue, userInfo1, idValue) {
    ...
    listItem.update();    
    context.executeQueryAsync(function OnSuccessAddItem(sender, args) {
        alert("Item Updated Successfully in JSOM");
        window.location.href = _spPageContextInfo.webAbsoluteUrl + "/Lists/MessageList/AllItems.aspx";
    },
        function OnFailureAddItem(sender, args) {
            console.log("Message :" + args.get_message() + "/n Trace :" + args.get_stackTrace());
