...
blankapp.controller("NewRequest", ['$scope', '$timeout', '$q', function ($scope, $timeout, $q) {
    ...
    $scope.requestfor="Self";
    $scope.RequestID=UniqueIDFormat();  
    $scope.onrequestChange=function(){
        if($scope.requestfor=="Others")
        {
            self.Employee=[];
            self.Managers=[]; 
            self.readonly=false;       
...
function UniqueIDFormat() {
    var dt = new Date();
    var time = "IAccessReqID" + dt.getDate() + "" + dt.getFullYear() + "" + dt.getHours() + "" + dt.getMinutes();
    return time;
}
